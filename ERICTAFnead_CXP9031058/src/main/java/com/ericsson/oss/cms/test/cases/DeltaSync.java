 ...
 * @author egergro
 */
public class DeltaSync extends TorTestCaseHelper implements TestCase {
    ...
    private static final int SLEEP_THREAD_50_SECONDS = 50;
     ...
     * @PRIORITY HIGH
     */
    @TestId(id = "OSS-53410_DeltaSync", title = "Perform DeltaSync on node")
    @Context(context = { Context.CLI })
    @DataDriven(name = "deltasync")
    @Test(groups = { "NEAD", "KGB" })
    public void nodeDeltaSync(
	...
	setTestInfo("Generation counter value before delta sync : " + genCounterBeforeDeltaSync);

 	if (genCounterBeforeDeltaSync == 0) {
            setTestInfo("Trying to Change attribute(GC value) for nodeType=" + nodeType + " as its GC is 0.");
            cppAvcOperatorCdb.setAttributeValues(moChildFdn, attributes[0], attributeValueGC);
            sleep(SLEEP_THREAD_50_SECONDS);
            final int gcAfterSetAttr = nodeOperator.getGenerationCounter(syncedNodeFdn);
            setTestInfo("Generation counter value after setting test value and wait for " + SLEEP_THREAD_50_SECONDS + " seconds : " + gcAfterSetAttr);
        }
	...
	setTestInfo("Generation counter value after delta sync : " + genCounterAfterDeltaSync);
